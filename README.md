...
- **Step 4 (현재):** 백지 그림판에 손그림풍 다구를 눌러 올리고 끌어서 자유 배치. 탭해 선택한 뒤 모서리 핸들로 크기 조절(배율 `s`). 위치·크기는 그림판 대비 비율로 저장.
...
TeaCard = {
  id,
  teaware: [{ ware, x, y, s }],    // x,y = 그림판 대비 비율(0..1), s = 크기 배율. (구형: string[] 이모지 — 호환 렌더)
